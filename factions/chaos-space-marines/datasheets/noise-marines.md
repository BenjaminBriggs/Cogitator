# Noise Marines

**Role:** Other

| M | T | Sv | W | Ld | OC |
|---|---|---|---|---|---|
| 6" | 5 | 3+ | 2 | 6+ | 1 |

**Invulnerable Save:** -

**Default Wargear:**
**Every model is equipped with:** sonic blaster; close combat weapon.

**Ranged Weapons**
| Name | Range | A | BS | S | AP | D | Keywords |
|---|---|---|---|---|---|---|---|
| Sonic blaster | 18 | 3 | 3 | 5 | -1 | 2 | ignores cover |
| Blastmaster – single frequency | 18 | 3 | 3 | 10 | -2 | 3 | ignores cover |
| Blastmaster – varied frequency | 18 | 6 | 3 | 6 | -2 | 1 | ignores cover |
| Screamer pistol | 12 | 3 | 3 | 5 | -1 | 2 | ignores cover, pistol |

**Melee Weapons**
| Name | Range | A | WS | S | AP | D | Keywords |
|---|---|---|---|---|---|---|---|
| Power sword | Melee | 4 | 3 | 5 | -2 | 1 | — |
| Close combat weapon | Melee | 3 | 3 | 4 | 0 | 1 | — |

**Abilities:**
- **Thrill Seekers** (Faction): If your Army Faction is Emperor’s Children, this unit is eligible to shoot and declare a charge in a turn in which it Advanced or Fell Back, but when doing so:
- It cannot target a unit it was within Engagement Range of at the start of the turn.
- It cannot target a unit that was the target of another units charge or attack this phase.
- **Terrifying Crescendo** (Datasheet): In your Shooting phase, after this unit has shot, select one enemy unit hit by one or more of those attacks. Until the start of your next Shooting phase, each time a Battle-shock or Leadership test is taken for that enemy unit, subtract 1 from that test.

**Keywords:** Noise Marines, Slaanesh, Chaos, Infantry
**Faction Keywords:** Emperor’s Children, Heretic Astartes

**Unit Composition:**
- 1 Disharmonist
- 5 Noise Marines

**Wargear Options:**
- The Disharmonist’s sonic blaster can be replaced with 1 screamer pistol and 1 power sword.
- Up to 2 Noise Marines can each replace their sonic blaster with 1 blastmaster.

**Points:**
- 6 models: 145pts
