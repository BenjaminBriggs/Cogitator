...
- **The Scorpion’s Bite** (Wargear): Melee weapons equipped by the bearer have the [DEVASTATING WOUNDS] ability when targeting units without the TITANIC keyword.
- **Shadow Hunter** (Datasheet): While this model is leading a unit, each time a model in that unit makes an attack, add 1 to the Hit roll.
- **Sustained Assault** (Datasheet): Each time this model makes a melee attack, if it made a Charge move this turn, a successful unmodified Hit roll of 4+ scores a Critical Hit.

**Keywords:** Epic Hero, Phoenix Lord, Aeldari, Karandras, Character, Infantry
