# Wazbom Blastajet

**Role:** Other

*Some Meks take to the skies in their own kustom aircraft. The result is the Wazbom Blastajet, an airborne workshop that is wreathed in a protective force field, and bristling with weird weapons that pluck their targets from the ground, blast them to atoms or rip them apart with conflicting tellyporta beams.*

| M | T | Sv | W | Ld | OC |
|---|---|---|---|---|---|
| 20+" | 9 | 3+ | 12 | 7+ | 0 |

**Invulnerable Save:** 6

**Default Wargear:**
**This model is equipped with:** smasha gun; twin wazbom mega-kannon; armoured hull.

**Ranged Weapons**
| Name | Range | A | BS | S | AP | D | Keywords |
|---|---|---|---|---|---|---|---|
| Smasha gun | 48 | D3 | 4 | 9 | -3 | 4 | blast |
| Twin wazbom mega-kannon | 36 | D3 | 4 | 12 | -2 | D6 | blast, hazardous, twin-linked |
| Twin supa-shoota | 36 | 4 | 4 | 6 | -1 | 1 | rapid fire 2, sustained hits 1, twin-linked |
| Twin tellyport mega-blasta | 24 | 3 | 5 | 9 | -1 | D6+1 | blast, twin-linked |

**Melee Weapons**
| Name | Range | A | WS | S | AP | D | Keywords |
|---|---|---|---|---|---|---|---|
| Armoured hull | Melee | 3 | 4 | 6 | 0 | 1 | — |

**Abilities:**
- **Deadly Demise** (Core): DEADLY DEMISEFrom detonating ammo stores to corrosive innards or frenzied death throes, some targets are deadly even in defeat.

Some models have ‘Deadly Demise x’ listed in their abilities. When such a model is destroyed, roll one D6 before removing it from play (if such a model is a TRANSPORT, roll before any embarked models disembark). On a 6, each unit within 6" of that model suffers a number of mortal wounds denoted by ‘x’ (if this is a random number, roll separately for each unit within 6").

**Example:** A TRANSPORT model with the Deadly Demise D3 ability is destroyed. Before any models disembark and before removing it from play, its controlling player rolls one D6, getting a 6. There are three units within 6" of that destroyed model, so its controlling player rolls one D3 for each of them, inflicting mortal wounds accordingly.
- **Deadly Demise x:** When this model is destroyed, roll one D6. On a 6, each unit within 6" suffers ‘x’ mortal wounds.
- **Waaagh!** (Faction): If your Army Faction is ORKS, once per battle, at the start of your Command phase, you can call a Waaagh!. If you do, until the start of your next Command phase, the Waaagh! is active for your army and:
- Units from your army with this ability are eligible to declare a charge in a turn in which they Advanced.
- Add 1 to the Strength and Attacks characteristics of melee weapons equipped by models from your army with this ability.
- Models from your army with this ability have a 5+ invulnerable save.
- **Blastajet Attack Run** (Datasheet): Each time this model makes a ranged attack that targets a unit that cannot FLY, re-roll a Hit roll of 1.
- **Blastajet Force Field** (Wargear): The bearer has a 4+ invulnerable save, but it loses the Grenades keyword.

**Keywords:** Aircraft, Fly, Wazbom Blastajet, Speed Freeks, Grenades, Vehicle
**Faction Keywords:** Orks

**Unit Composition:**
- 1 Wazbom Blastajet

**Wargear Options:**
- This model’s twin wazbom mega-kannon can be replaced with 1 twin tellyport mega-blasta.
- This model can be equipped with 1 blastajet force field.
- This model can be equipped with 1 twin supa-shoota.

**Damaged Profile:**
Remaining Wounds: 1-4
While this model has 1-4 wounds remaining, each time this model makes an attack, subtract 1 from the Hit roll.

**Points:**
- 1 model: 175pts
