# Corsair Reaver Band

**Role:** Other

| M | T | Sv | W | Ld | OC |
|---|---|---|---|---|---|
| 7" | 3 | 5+ | 3 | 6+ | 2 |

**Invulnerable Save:** -

**Default Wargear:**
**Every model is equipped with:** brace of pistols; Corsair firearm; close combat weapon.

**Ranged Weapons**
| Name | Range | A | BS | S | AP | D | Keywords |
|---|---|---|---|---|---|---|---|
| Aeldari missile launcher – starshot | 48 | 1 | 3 | 10 | -2 | D6 | — |
| Aeldari missile launcher – sunburst | 48 | D6 | 3 | 4 | -1 | 1 | BLAST |
| Blaster | 18 | 1 | 3 | 8 | -4 | D6+1 | assault |
| Brace of pistols | 12 | 2 | 3 | 3 | 0 | 1 | assault, pistol |
| Corsair firearm | 24 | 4 | 3 | 4 | 0 | 1 | assault, lethal hits |
| Dark lance | 36 | 1 | 4 | 12 | -3 | D6+2 | heavy |
| Dissonance pistol | 12 | 1 | 3 | 6 | -1 | 1 | devastating wounds, pistol |
| Flamer | 12 | D6 | N/A | 4 | 0 | 1 | assault, ignores cover, torrent |
| Fusion gun | 12 | 1 | 3 | 8 | -4 | D6+1 | assault, melta 2 |
| Shredder | 18 | D6 | N/A | 6 | 0 | 1 | assault, torrent |
| Shuriken cannon | 24 | 3 | 3 | 6 | -1 | 2 | sustained hits 1 |
| Splinter cannon | 36 | 3 | 3 | 3 | -1 | 2 | anti-infantry 3+, sustained hits 1 |

**Melee Weapons**
| Name | Range | A | WS | S | AP | D | Keywords |
|---|---|---|---|---|---|---|---|
| Close combat weapon | Melee | 2 | 3 | 3 | 0 | 1 | — |
| Spar-glaive | Melee | 4 | 3 | 3 | 0 | 1 | — |
| Void sabre | Melee | 3 | 3 | 4 | -2 | 1 | — |

**Abilities:**
- **Reaver Band** (Datasheet): Once per turn, when an enemy unit ends a Normal, Advance or Fall Back move within 9" of this unit, if this unit is not within Engagement Range of one or more enemy units, it can make a Normal move of up to D6".
- **CORSAIRS** (Special (правая колонка)): If a model from your army with the Leader ability can be attached to a Corsair Voidreavers unit, it can be attached to this unit instead.

**Keywords:** Grenades, Anhrathe, Corsair Reaver Band, Infantry, Aeldari
**Faction Keywords:** Asuryani, 

**Unit Composition:**
- 0-1 Corsair Reaver Felarch
- 5-10 Corsair Reavers

**Wargear Options:**
- Any number of models can each have their Corsair firearm replaced with 1 spar-glaive.
- For every 5 models in this unit, 1 model’s Corsair firearm can be replaced with one of the following:- 1 Aeldari missile launcher
- 1 blaster
- 1 dark lance
- 1 flamer
- 1 fusion gun
- 1 shredder
- 1 shuriken cannon
- 1 splinter cannon
- The Corsair Reaver Felarch can be equipped with one of the following:- 1 dissonance pistol
- 1 void sabre

**Points:**
- 5 models: 125pts
- 6 models: 150pts
- 10 models: 250pts
- 11 models: 275pts
